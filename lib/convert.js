...
class Swagger2Postman {
    ...
    setValidate(cb) {
        ...
        validator.create(function (err, func) {
            if (err) {
                self.logger('failed to load schema: ' + err.message);
                cb();
                return;
            }
            self.validate = func;
    ...
    getModelTemplate(model) {
        ...
        if (model.type === 'object' || model.properties) {
            ...
            value = this.getModelTemplate(model.items);
            return JSON.stringify(JSON.parse('[' + value + ']'), null, 4);
        }

        return this.getDefaultValue(model.type);
    }
    ...
    buildItemFromOperation(path, method, operation, paramsFromPathItem) {
        ...
        if (operation[constants.META_KEY]) {
            if (operation[constants.META_KEY].hasOwnProperty('auth')) {
                request = this.applyPostmanSecurity(operation[constants.META_KEY].auth, request);
            }
        }
